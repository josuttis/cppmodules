...
.ctt.ott:
	...
	then \
	    echo MIT INPUT; \
	    sed -f $(TTT2CPP) < $*.itt > tmp.in; \
	    make $* && echo "STARTE:  $$ARG" && ./$$ARG < tmp.in > $*.out; \
...
help::
	@echo 'progs:  alles in $$(PROGS) und alles in $$(OUTPROGS)'
...
progs::
	...
	then \
	    ...
	    do \
	        echo "MAKE $$PROG.ott"; \
	        make $$PROG.ott || echo " + make $$PROG.ott failed !!!" >> MAKE.LOG; \
	    done; \
	fi
	@if test -s MAKE.LOG ; \
	then \
...
help::
	@echo 'expand: alle tabs expandieren'
expand::
	@for DATEI in *.[cht][tx][tx]; \
	do \
	  if test -r $$DATEI; \
	  then \
	    if fgrep -q '	' $$DATEI; \
	    then \
		echo expand $$DATEI; \
		mv $$DATEI $$DATEI.unexpand; \
		expand < $$DATEI.unexpand > $$DATEI; \
	    fi; \
	  fi; \
	done

help::
	@echo 'check: fuer alle checkTTT aufrufen'
check::
	@for DATEI in *.[cht]tt; \
	do \
	  if test -r $$DATEI; \
...
headers:
	...
	do \
	  if test "$$FILE" != "" ; \
	  then \
	    BASE=`basename $$FILE .htt`; \
	    BASE=`basename $$FILE .hpp`; \
	    sed -f $(TTT2CPP) < $$BASE.htt > $$FILE; \
	  fi \
	done
...
############ clean and delete ##########################
help::
	@echo 'clean:  alles generierte bis auf Output-Dateien loeschen'
	@echo 'delete: ALLES generierte loeschen'
clean::
	rm -rf *.out *.in MAKE.LOG *.unexpand *.o *.exe *.ii *.ti *~
	rm -rf *.obj *.ifc gcm.cache
